...
mod test {
    ...
    fn test_sk() -> libsecp256k1::SecretKey {
        let dec_sk = hex::decode("6305ebd1c45068587419680294b2cc67e8116710bec85c4ca1fa7e6722e8be6a").unwrap();
    ...
    fn test_signed_instruction_serializoor() {
        ...
        assert_eq!(crate::signed_message::SignedInstructionSerializoor::serialize(&ix), serialized_ix);
    }
    ...
    fn test_bytesigned_ix_signed_instruction() {
        ...
        const EXPECTED_ENC_B_IX: &str = "41896cdf1cfa7c6a44b5ee18d57b32712ad86c69a76a3ae57957a0238b20a205";
        ...
        let expected_s_msg = SignedMessage {
            ...
            wallet_pubkey: WalletType::Ethereum.encode_key(libsecp256k1::PublicKey::from_secret_key(&sk)),
            recovery_id: rec_id.serialize()
        };
        let s_msg = b_ix.into_signed_message(SignedMessageOpts { 
            signature: sig.serialize(), 
            recovery_id: rec_id.serialize(), 
        ...
        assert_eq!(
            EXPECTED_RECOVERED_SIGNER,
            hex::encode(&recovered_signer.0)
        );

        let recovered = ByteSignedIx::convert_recovered_public_key(recovered_signer).unwrap();

        assert_eq!(EXPECTED_SERIALIZED_RECOVERED_SIGNER, hex::encode(recovered.serialize()));
    }
